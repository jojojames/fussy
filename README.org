...
We need to advise company to use ~fussy~ (NOTE: This only works with
~company-capaf~.)
...
#+end_src
* Corfu Integration
~corfu~ should work out of the box with ~fussy~. We can tweak it like this
though.

#+begin_src emacs-lisp :tangle yes
  (add-hook 'corfu-mode-hook
            (lambda ()
              (setq-local fussy-completion-at-point t
                          fussy-max-candidate-limit 3000)))
#+end_src
